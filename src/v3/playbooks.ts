...
function formatPlaybook(p: Playbook): string {
	const parts = [`Playbook: ${p.playbook_id}`, `Title: ${p.title}`]
	if (p.macro) {
		parts.push(`Macro: ${p.macro}`)
	}
...
export function registerPlaybookTools(server: McpServer): void {
	server.tool(
		...
		async (params) => {
			...
			const text = formatPaginatedResponse<Playbook>(data, formatPlaybook, 'No playbooks found.')
	...
	server.tool(
		...
		'Create a new playbook. Playbooks define reusable instruction sets for Devin sessions.',
		{
			...
			macro: z.string().nullable().optional().describe('Optional macro identifier (null to clear)'),
		},
		async (params) => {
