...
export function registerSecretTools(server: McpServer): void {
  server.tool(
    ...
    async (params) => {
      ...
      const text = formatPaginatedResponse<Secret>(
        data,
        formatSecret,
        "No secrets found."
      );
  ...
  server.tool(
    ...
    "Create a new secret in the organization. The value is encrypted at rest and never returned.",
    {
      ...
      is_sensitive: z
        .boolean()
        .optional()
        .describe("If true, value is redacted in logs (default: true)"),
      note: z
        .string()
        .nullable()
        .optional()
        .describe("Optional description of the secret's purpose"),
    },
    async (params) => {
      ...
      if (params.is_sensitive !== undefined) body.is_sensitive = params.is_sensitive;
