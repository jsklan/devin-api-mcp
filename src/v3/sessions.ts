...
import { z } from "zod";
import { v3Fetch, v3FetchArrayQuery, paginationParams, formatPaginatedResponse } from "./shared.js";
...
function formatSession(s: SessionResponse): string {
  ...
  if (s.created_at) parts.push(`Created: ${new Date(s.created_at * 1000).toISOString()}`);
  if (s.updated_at) parts.push(`Updated: ${new Date(s.updated_at * 1000).toISOString()}`);
  if (s.acus_consumed !== undefined) parts.push(`ACUs consumed: ${s.acus_consumed}`);
  ...
  if (s.is_advanced) parts.push(`Advanced: yes`);
  if (s.is_archived) parts.push(`Archived: yes`);
  if (s.playbook_id) parts.push(`Playbook: ${s.playbook_id}`);
  if (s.user_id) parts.push(`User: ${s.user_id}`);
  if (s.service_user_id) parts.push(`Service user: ${s.service_user_id}`);
  if (s.parent_session_id) parts.push(`Parent session: ${s.parent_session_id}`);
  if (s.child_session_ids?.length) parts.push(`Child sessions: ${s.child_session_ids.join(", ")}`);
  if (s.structured_output) parts.push(`Structured output:\n${JSON.stringify(s.structured_output, null, 2)}`);
  return parts.join("\n");
}
...
function buildSessionFilterQuery(params: Record<string, unknown>): Record<string, string | number | boolean | string[] | undefined | null> {
  const query: Record<string, string | number | boolean | string[] | undefined | null> = {};
  if (params.after) query.after = params.after as string;
  if (params.first) query.first = params.first as number;
  if (params.session_ids) query.session_ids = params.session_ids as string[];
  if (params.created_after) query.created_after = params.created_after as number;
  if (params.created_before) query.created_before = params.created_before as number;
  if (params.updated_after) query.updated_after = params.updated_after as number;
  if (params.updated_before) query.updated_before = params.updated_before as number;
  if (params.tags) query.tags = params.tags as string[];
  if (params.playbook_id) query.playbook_id = params.playbook_id as string;
  if (params.origins) query.origins = params.origins as string[];
  if (params.schedule_id) query.schedule_id = params.schedule_id as string;
  if (params.user_ids) query.user_ids = params.user_ids as string[];
  if (params.service_user_ids) query.service_user_ids = params.service_user_ids as string[];
  return query;
}
...
export function registerSessionTools(server: McpServer): void {
  ...
  server.tool(
    ...
    "Create a new Devin session via the v3 API. Sends a prompt to Devin and returns the session ID and URL.",
    {
      ...
      knowledge_ids: z.array(z.string()).optional().describe("Knowledge IDs to attach. Omit to use all, pass empty array to use none"),
      secret_ids: z.array(z.string()).optional().describe("Secret IDs to attach. Omit to use all, pass empty array to use none"),
      session_secrets: z
        .array(
          z.object({
    ...
    async (params) => {
      ...
      if (params.playbook_id !== undefined) body.playbook_id = params.playbook_id;
      if (params.knowledge_ids !== undefined) body.knowledge_ids = params.knowledge_ids;
      if (params.secret_ids !== undefined) body.secret_ids = params.secret_ids;
      if (params.session_secrets !== undefined) body.session_secrets = params.session_secrets;
      if (params.max_acu_limit !== undefined) body.max_acu_limit = params.max_acu_limit;
      if (params.structured_output_schema !== undefined) body.structured_output_schema = params.structured_output_schema;
      if (params.attachment_urls !== undefined) body.attachment_urls = params.attachment_urls;
      if (params.repos !== undefined) body.repos = params.repos;
      if (params.advanced_mode !== undefined) body.advanced_mode = params.advanced_mode;
      if (params.bypass_approval !== undefined) body.bypass_approval = params.bypass_approval;
      if (params.create_as_user_id !== undefined) body.create_as_user_id = params.create_as_user_id;
      if (params.child_playbook_id !== undefined) body.child_playbook_id = params.child_playbook_id;
      if (params.session_links !== undefined) body.session_links = params.session_links;
  ...
  server.tool(
    ...
    async (params) => {
      ...
      const text = formatPaginatedResponse<SessionResponse>(
        data,
        formatSession,
        "No sessions found."
      );
  ...
  server.tool(
    ...
    async ({ devin_id, archive }) => {
      ...
      if (archive !== undefined) query.archive = archive;
  ...
  server.tool(
    ...
    async ({ devin_id, message, message_as_user_id }) => {
      ...
      if (message_as_user_id !== undefined) body.message_as_user_id = message_as_user_id;
  ...
  server.tool(
    ...
    async ({ devin_id, after, first }) => {
      ...
      const text = formatPaginatedResponse<SessionMessage>(
        data,
        (msg) =>
          `[${msg.source}] (${new Date(msg.created_at * 1000).toISOString()}) ${msg.message}`,
        "No messages found."
      );
      return { content: [{ type: "text" as const, text }] };
    }
        ...
        content: [
          {
            type: "text" as const,
            text: tags.length
              ? `Tags for session ${devin_id}: ${tags.join(", ")}`
              : `Session ${devin_id} has no tags.`,
          },
        ],
      };
    }
  ...
  server.tool(
    ...
    async (params) => {
      ...
      const text = formatPaginatedResponse(
        data,
        formatSessionInsights,
        "No session insights found."
      );
      return { content: [{ type: "text" as const, text }] };
    }
  );
  ...
  server.tool(
    ...
    async ({ devin_id }) => {
      ...
      const attachments: SessionAttachment[] = Array.isArray(result) ? result : result?.items || result?.attachments || [];
...
function formatSessionInsights(s: SessionInsights): string {
  ...
  if (s.num_user_messages !== undefined) parts.push(`User messages: ${s.num_user_messages}`);
  if (s.num_devin_messages !== undefined) parts.push(`Devin messages: ${s.num_devin_messages}`);
  if (s.session_size) parts.push(`Session size: ${s.session_size}`);
  if (s.analysis) parts.push(`Analysis:\n${s.analysis}`);
  return parts.join("\n");
}
