...
function formatSchedule(s: ScheduleResponse): string {
	...
	if (s.frequency) {
		parts.push(`Frequency: ${s.frequency}`)
	}
	if (s.scheduled_at) {
		parts.push(`Scheduled at: ${s.scheduled_at}`)
	}
	if (s.playbook) {
		parts.push(`Playbook: ${s.playbook.title} (${s.playbook.playbook_id})`)
	}
	parts.push(`Notify on: ${s.notify_on}`)
	if (s.last_executed_at) {
		parts.push(`Last executed: ${s.last_executed_at}`)
	}
	if (s.last_error_message) {
		parts.push(`Last error: ${s.last_error_message} (at ${s.last_error_at})`)
	}
	if (s.consecutive_failures > 0) {
		parts.push(`Consecutive failures: ${s.consecutive_failures}`)
	}
	if (s.created_by) {
		parts.push(`Created by: ${s.created_by}`)
	}
	parts.push(`Created: ${s.created_at} | Updated: ${s.updated_at}`)
	if (s.slack_channel_id) {
		parts.push(`Slack channel: ${s.slack_channel_id}`)
	}
	if (s.slack_team_id) {
		parts.push(`Slack team: ${s.slack_team_id}`)
	}
	return parts.join('\n')
}
	...
	server.tool(
		...
		'Create a new scheduled Devin session. Use frequency (cron) for recurring or scheduled_at (ISO 8601) for one-time.',
		{
			...
			frequency: z
				.string()
				.optional()
				.describe("Cron expression for recurring schedules (e.g. '0 9 * * 1' for every Monday at 9am)"),
			...
			notify_on: z
				...
				.optional()
				.describe('When to send notifications (default: failure)'),
		...
		async (params) => {
			...
			if (params.schedule_type) {
				body.schedule_type = params.schedule_type
			}
			if (params.frequency) {
			...
			if (params.notify_on) {
				body.notify_on = params.notify_on
			}
			if (params.create_as_user_id) {
				body.create_as_user_id = params.create_as_user_id
			}
			if (params.slack_channel_id) {
				body.slack_channel_id = params.slack_channel_id
			}
			if (params.slack_team_id) {
				body.slack_team_id = params.slack_team_id
			}
