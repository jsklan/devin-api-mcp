...
export function formatPaginatedResponse<T>(
  ...
  if (!data.items?.length) return emptyMessage;

  const lines = data.items.map(formatItem);
  const meta: string[] = [];
  if (data.total !== null && data.total !== undefined) meta.push(`Total: ${data.total}`);
  if (data.has_next_page && data.end_cursor) meta.push(`Next cursor: ${data.end_cursor}`);
