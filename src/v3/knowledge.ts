...
function formatKnowledgeNote(k: KnowledgeNote): string {
  ...
  if (k.pinned_repo) parts.push(`Repo: ${k.pinned_repo}`);
  if (k.folder_id) parts.push(`Folder: ${k.folder_id}`);
  return parts.join("\n");
}
...
export function registerKnowledgeTools(server: McpServer): void {
  server.tool(
    ...
    async (params) => {
      ...
      const text = formatPaginatedResponse<KnowledgeNote>(
        data,
        formatKnowledgeNote,
  ...
  server.tool(
    ...
    async (params) => {
      ...
      if (params.pinned_repo !== undefined) body.pinned_repo = params.pinned_repo;
  ...
  server.tool(
    ...
    "Update an existing knowledge note by ID.",
    {
      ...
      pinned_repo: z
        ...
        .nullable()
        .optional()
        .describe("Updated pinned repo (null to clear)"),
    },
    async ({ note_id, ...fields }) => {
      ...
      if (fields.pinned_repo !== undefined) body.pinned_repo = fields.pinned_repo;
